  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 pb-4 sm:pb-6 border-b border-gray-100">
        <div>
          <h3 className="text-lg sm:text-xl font-black text-[#1A1A1A]">Manage Doctors</h3>
      ...
      {(createMutation.error || deleteMutation.error) && (
        <div className="bg-red-50 border border-red-100 rounded-xl p-4 text-red-500 text-sm">
          ...
          <p>{(createMutation.error as any)?.message || (deleteMutation.error as any)?.message || 'Something went wrong'}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
        {doctors?.length === 0 && (
          <div className="text-center py-12 bg-gray-50 rounded-2xl col-span-full">
            <p className="text-gray-400 font-medium">No doctors yet. Add your first doctor!</p>
          </div>
        )}
        {doctors?.map((doctor) => (
          <div
          ...
          >
            ...
            <div className="flex-1 space-y-2">
              <span className="text-xs font-black text-[#00A78E] uppercase tracking-widest flex items-center">
            ...
            <div className="absolute top-6 right-6 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-all">
              <button
                ...
                className="p-4 bg-white/90 backdrop-blur-sm text-[#00A78E] rounded-2xl shadow-xl hover:scale-110 transition-all"
              >
                <Edit2 size={20} />
              </button>
              <button
                ...
                className="p-4 bg-white/90 backdrop-blur-sm text-red-400 hover:text-red-500 rounded-2xl shadow-xl hover:scale-110 transition-all"
              >
                <Trash2 size={20} />
              </button>
            </div>
